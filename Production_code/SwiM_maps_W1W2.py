...
with open('/Users/Nikhil/code/Newtext/Matchtxt/W2W1TOT.txt') as f:
   Line = [line.rstrip('\n') for line in open('/Users/Nikhil/code/Newtext/Matchtxt/W2W1TOT.txt')]
...
#CONVOLUTION START (Emission Lines/Fluxes/EWs/Ivars)
    ...
    FM2 = np.ones(np.shape(ND4_R))*(-1)
    FEM2 = np.ones(np.shape(ND4_R))*(-1)
    CM2 = np.ones(np.shape(ND4_R))*(-1)
    EM2 = np.ones(np.shape(ND4_R))*(-1)
    EXM2 = np.ones(np.shape(ND4_R))*(-1)
    MKM2 = np.ones(np.shape(ND4_R))*(-1)
